...
/**
 * Builds a [JsonArray] with the specified size, using the provided initializer function
 * to generate each element.
 *
 * @param size The number of elements in the resulting [JsonArray].
 * @param initializer A lambda function that takes an index and returns a [JsonElement].
 * @return A [JsonArray] containing elements generated by the initializer function.
 *
 * @see kotlinx.serialization.json.buildJsonArray
 */
fun buildJsonArray(size: Int, initializer: (idx: Int) -> JsonElement): JsonArray {
    return buildJsonArray { repeat(size) { idx -> add(initializer(idx)) } }
}
